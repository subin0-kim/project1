...
namespace Mukseon.UI
{
    ...
    internal sealed class SettingsColorSection
    {
        ...
        // 스와치 앞 4개가 팔레트에서 나오므로 목록도 팔레트에 의존한다. 격자 조립과 선택 상태 갱신이
        // 반드시 같은 목록을 봐야 해서(다르면 선택 링이 엉뚱한 칸에 붙는다) 생성자에서 한 번만 만든다.
        private readonly IReadOnlyList<SettingsScreenContent.ColorSwatch> _swatches;
        ...
        public SettingsColorSection(VisualElement parent, DirectionColorPalette palette, Action onAssigned)
        {
            ...
            _swatches = SettingsScreenContent.BuildSwatches(palette);
        ...
        public void Refresh()
        {
            for (int r = 0; r < _rows.Count; r++)
            {
                ...
                for (int s = 0; s < row.Swatches.Length; s++)
                {
                    ...
                    bool selected = SameColor(_swatches[s].Color, current);
        ...
        private Row BuildRow(VisualElement parent, SettingsScreenContent.DirectionRow definition)
        {
            ...
            var buttons = new Button[_swatches.Count];
            for (int i = 0; i < _swatches.Count; i++)
            {
                buttons[i] = BuildSwatch(row, definition.Direction, _swatches[i]);
            }
